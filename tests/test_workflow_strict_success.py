...
class WorkflowStrictSuccessTests(unittest.TestCase):
    ...
    def test_catalog_winner_is_unambiguous_in_actions_job_list(self):
        attempts = self.jobs["catalog_attempts"]
        self.assertIn("Catalog probe", attempts["name"])
        self.assertEqual(
            attempts["outputs"]["winner"],
            "${{ steps.existing.outputs.winner || steps.fetch.outputs.winner }}",
        )
        result = self.jobs["catalog_result"]
        self.assertEqual(
            result["name"],
            "✅ Catalog fetched — Runner ${{ needs.catalog_attempts.outputs.winner }}/10",
        )
        self.assertEqual(result["needs"], "catalog_attempts")

    def test_catalog_artifact_identifies_winner_and_setup_consumes_it(self):
        attempts = self.jobs["catalog_attempts"]
        upload = next(
            step for step in attempts["steps"]
            if step.get("name") == "Upload successful catalog bootstrap"
        )
        self.assertEqual(
            upload["with"]["name"],
            "catalog-bootstrap-runner-${{ matrix.attempt }}",
        )
        download = next(
            step for step in self.jobs["setup"]["steps"]
            if step.get("name") == "Download catalog generated by the first successful runner"
        )
        self.assertEqual(download["with"]["pattern"], "catalog-bootstrap-runner-*")
        self.assertTrue(download["with"]["merge-multiple"])
        self.assertEqual(set(self.jobs["setup"]["needs"]), {"catalog_attempts", "catalog_result"})
    ...
    def test_resume_planner_runs_after_setup_when_catalog_jobs_are_skipped(self):
        planner = self.jobs["resume_planner"]
        self.assertEqual(planner["needs"], "setup")
        self.assertEqual(planner["if"], "always() && needs.setup.result == 'success'")
    ...
    def test_setup_falls_back_when_resume_shared_config_is_missing(self):
        ...
        self.assertIn("steps.restore_shared_config.outputs.restored != 'true'", catalog_step.get("if", ""))
