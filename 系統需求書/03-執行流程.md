...
- **跨 Run 續作的唯一固定順序**：上一 Run artifacts → GitHub Actions Cache → 重新製作；不得交換，也不得同時競速讀取。
- 調度器先以正規化目錄 URL 的穩定 `book_profile_id` 找到同指紋最新已結束 Run，將唯一 ID 同時持久寫入任務的 `artifact_source_run_id` 並傳入 workflow 的 `resume_source_run_id`。Worker 不得依書名、task ID 或最近 Run 自行掃描猜測來源。
- Worker 先下載鎖定 Run 的 `shared-config` 與自己的 `video-worker-N`，驗證 `shared-config.book_profile_id` 與本次完全一致，再用 checkpoint 驗證所有指定章節。artifact 完整時必須跳過 Cache restore，pipeline 不得重做任何已驗收 stage。
- artifact 不存在或逐章驗收不完整時，才可執行 Cache fallback。Cache key 與 restore prefix 必須包含相同 `book_profile_id`、書名、章號範圍及 worker ID，禁止同名不同來源互相污染；Cache 後仍缺失的章節才可重新製作。
- artifact 存在但下載失敗、來源設定缺失、指紋不符或內容損壞時必須 fail closed，不得把錯誤偽裝成 artifact 不存在後靜默改用 Cache。
- Actions 日誌必須顯示鎖定來源 Run、指紋驗證、artifact 完整度及 Cache fallback 是否獲准。自動化測試必須鎖定順序、不同指紋拒絕繼承、完整 artifact 禁止 Cache，以及 pipeline 不再自行掃描歷史 Run。
