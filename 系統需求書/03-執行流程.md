...
sequenceDiagram
    ...
    par 最多同時 17 workers
        W->>C: 由 queue 同指紋 run_history 新到舊回溯 artifacts
...
- **跨 Run 續作的唯一固定順序**：同指紋全部歷史 Run artifacts（新到舊）→ GitHub Actions Cache → 只重新製作仍缺少的 stage；不得交換，也不得同時競速讀取。
- Worker 必須先讀取 `automation-state:audiobook-queue.json`，以本次非空 `book_profile_id` 精確選出 `queue` 與 `completed` 中同指紋任務的 `run_history`，跨任務合併、去重並依 `ended_at` 新到舊排序。書名、task ID、Run 顯示名稱及「最近 Run」都不得作為 artifact 歸屬或候選搜尋條件。
...
- 全部歷史 Run 耗盡後仍不完整時，才可執行 Cache fallback。Cache key 與 restore prefix 必須包含相同 `book_profile_id`、書名、章號範圍及 worker ID；Cache 後仍缺失的章節／stage 才可重新製作。
