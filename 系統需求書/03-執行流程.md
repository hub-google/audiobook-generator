...
sequenceDiagram
    ...
    par 最多同時 17 workers
        W->>C: 只還原調度器鎖定的單一紅 X 前手 artifacts
        W->>W: 每個候選 Run 二次驗證指紋與逐章完整性
        opt 歷史 artifacts 全部耗盡後仍不完整
            W->>C: 以相同書籍指紋還原 Workspace cache
        end
        loop 每個分配章節
...
- **跨 Run 續作的唯一固定順序**：調度器只可鎖定同指紋、已結束且 `conclusion=failure`（GitHub 紅色 X）、Run 與必要 artifacts 仍存在的最新單一 Run。`cancelled`、`missing`、執行中與已刪除 Run 一律不得使用。鎖定後只可下載該 Run，最多重試 3 次，不得再掃描或混用更舊 Run；可驗收成果全部沿用，缺失或不合格 stage 才依序讀取精確 Cache 或重新製作。若鎖定來源完全不可用或連續 3 次下載失敗，直接放棄恢復並重新製作。
- 調度器必須讀取 `automation-state:audiobook-queue.json`，以本次非空 `book_profile_id` 從 `queue` 與 `completed` 的 `run_history` 中篩選 `conclusion=failure`，再用 GitHub API 排除已刪除、未結束或必要 artifacts 不存在的 Run，鎖定最新單一來源。Worker 不得自行搜尋其他歷史 Run。
- 對每個候選 Run，先下載 `shared-config` 作第二層一致性驗證，再下載自己的 `video-worker-N`（完整 Workspace）及必要時 `mp4-worker-N`。Run 404、artifact 不存在或尚未上傳、`shared-config` 缺失、指紋矛盾、artifact 不完整時，都必須記錄原因並繼續更舊 Run，不得提前轉入 Cache。
- 每次成功下載都須合併有效檔案並以 checkpoint 重新驗證；部分 artifact 的有效章節／stage 必須保留，再由更舊 Run 補缺口。只有 Worker 已完整，或同指紋 `run_history` 全部耗盡，才能停止歷史回溯。
- 唯一來源最多下載 3 次；下載失敗或驗收後完全不可用時直接執行 Cache fallback。Cache key 與 restore prefix 必須包含相同 `book_profile_id`、書名、章號範圍及 worker ID；Cache 後仍缺失的章節／stage 才可重新製作，且不得改抓其他歷史 Run。
- 前手 `prepared-plan` 若與目前書籍指紋、章節清單及 cleaner fingerprint 完全相符，必須直接沿用；每個 `merge-result-*` 必須依鎖定 plan 的 `source_run_id`、Part 集合與字幕驗收。通過的 Merge shard 不得重做，只有缺失或不合格 shard 才執行 Merge。
- `shared-config` 不是尋找同書 Run 的索引；它是 queue 已選定候選後的防污染二次驗證。queue 記錄與 `shared-config.book_profile_id` 不一致時必須拒絕該 Run 並繼續更舊候選。
- Actions 日誌必須顯示完整候選順序、每個 Run 的 404／缺 artifact／指紋驗證結果、累積完整度，以及何時允許 Cache。測試必須涵蓋 9→1 全部耗盡、跨任務同指紋排序、不同指紋排除、部分 artifact 累積與完整後提早停止。
