...
雲端媒體 runtime 是第五個共用基礎邊界：`Dockerfile.worker` 必須預裝 Python 3.11、Python requirements、FFmpeg/ffprobe、CJK fonts、GitHub CLI 及 Actions cache/artifact 需要的壓縮工具，並發布至 `ghcr.io/hub-google/audiobook-worker:v1`。`setup`、`process_chapters`、`plan_parts`、`merge_parts` 與 `upload_to_youtube` 必須共用這份 image，不得在每個 job 內重複執行 apt/pip 安裝。
