...
flowchart LR
    ...
    GH --> WF["audiobook.yml"]
    CR["GHCR audiobook-worker:v1"] --> WF
...
雲端媒體 runtime 是第五個共用基礎邊界：`Dockerfile.worker` 必須預裝 Python 3.11、Python requirements、FFmpeg/ffprobe、CJK fonts、GitHub CLI 及 Actions cache/artifact 需要的壓縮工具，並發布至 `ghcr.io/hub-google/audiobook-worker:v1`。`setup`、`process_chapters`、`plan_parts`、`merge_parts` 與 `upload_to_youtube` 必須共用這份 image 的不可變 digest，不得在每個 job 內重複執行 apt/pip 安裝。

## 2. 模組責任

| 模組 | 唯一責任 | 主要輸入 | 主要輸出 |
|---|---|---|---|
| `gui_app.py` | 桌面操作、Cleaner 純文字刪除設定與一致性預覽、dispatch、cancel、輪詢與 log 呈現 | URL、章節範圍、排除章、每書刪除文字、`.env` | GitHub workflow run |
| `book_profiles.py` | 驗證並持久化每書設定；產生 Cleaner 語意版本指紋 | 目錄 URL、純文字刪除清單、章名／章號覆寫 | `audiobook-book-profiles.json` profile/snapshot |
...
| `cleaner.py` | 逐字刪除每書指定文字、去雜訊、句子切分、字幕友善行長 | RawText、純文字刪除清單 | `CleanText/*_clean.txt` |
...
- GitHub Container Registry（GHCR）：保存預建的雲端媒體 runtime；只有 Dockerfile、requirements 或 image workflow 變更時才重建。
