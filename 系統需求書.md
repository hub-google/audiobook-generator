...
│       └── youtube_stream.yml   # YouTube Live RTMP 直播推流工作流
└── src/
    ...
    ├── metadata_gen.py          # AI 封面與 YouTube 元數據生成器（網路大綱提取、英文 Prompt 翻譯、Flux 2K AI 封面繪製與排版）
    ├── video_gen.py             # FFmpeg 章節獨立影片極速渲染 + concat Copy 秒級合併 + metadata 時間戳生成
    ├── subtitle_gen.py          # SRT 字幕生成與合併器（精準計算音訊分段時間軸，產出單章與全書 SRT）
    ├── stream_to_youtube.py     # YouTube Live 超長直播推流與自動 Metadata 上傳器
...
* **`src/stream_to_youtube.py` (YouTube Live 超長直播推流與自動 Metadata 上傳器)**：
  - **多通道 Stream Key 廣播池與自動避讓**：自動讀取 `YOUTUBE_STREAM_KEY`, `KEY_2`, `KEY_3`（共 3 組 Key 備選池，最多可擴充至 5 組），推流前以 2 秒輕量級 Probe 偵測，若發現某組 Key 已被佔用則自動跳過切換下一組，防止將線上正在播出的直播踢下線。
  - **雙線程異步預下載 (Async Prefetching)**：主線程推流播送當前 Worker 的同時，背景執行緒同步預先下載下一個 Worker Artifact，實現 0 秒等待的極速串流。
  - **單 Worker 無損 Concat 串流**：同 Worker 內的所有章節 MP4 檔先於 1 秒內經由 FFmpeg `-c copy` 無損合併為單一區塊再推流，大幅降低重覆連線與握手開銷。
  - **雙緩衝硬碟記憶體極限控管**：採用 Ping-Pong `dir_a` / `dir_b` 雙資料夾輪替，單個 Worker 推流完畢立即執行 `shutil.rmtree()` 徹底清空，確保硬碟空間永久封頂於 2GB 以內。
  - **雙重嚴格章節排序**：Worker 序號與檔名內章節數字進行嚴格整數解析排序，100% 保證按第 1 至 2442 章正序連續播送。
