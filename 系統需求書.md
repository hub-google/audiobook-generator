...
1. **視覺解析度 (2560 × 1440 2K QHD)**：使用 Pollinations AI (Flux 大模型) 下載 2K 背景繪圖底圖。
2. **主角與背景構圖**：指定 AI 生成「左側主角姿態 + 右側淡色背景」，右側留出大氣空間襯托標題。
3. **自適應右對齊大字**：
   - 6字以內（如《凡人修仙傳》）：210pt 巨型飛白狂草大字。
   - 7~11字：155pt 自動分為兩行右對齊。
   - 12~20字：115pt 依標點符號自動分為兩至三行海報式右對齊。
4. **右下角醒目完結徽章**：高對比翡翠綠 `【 已完結 】` 徽章，距離底部 140px。
