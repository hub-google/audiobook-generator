...
def confirm_preflight_review(queue, task_id, scrape_run_id, cover_run_id, review):
    ...
    if (task.get("status") != "waiting_review" or
            ...
            any(task["stages"][name]["status"] != "completed" for name in ("scrape", "cover"))):
        raise ValueError("TXT／封面尚未完成或雲端任務已變更，請重新同步並審核")
    task.update(ad_review=copy.deepcopy(review))
    return touch(queue)


def start_reviewed_processing(queue, task_id):
    """Move a previously confirmed review into the processing queue."""
    queue = normalize_queue(queue)
    task = next(t for t in queue["queue"] if t["task_id"] == task_id)
    review = task.get("ad_review") or {}
    if review.get("status") != "approved":
        raise ValueError("請先確認文字清理設定")
    return approve_preflight(
        queue, task_id, task.get("scrape_run_id"), task.get("cover_run_id"), review,
    )
...
def update_task_chapters(queue, task_id, start_chapter, end_chapter, excluded_chapters=None,
    ...
    if catalog_identity is not None:
        changes['catalog_identity'] = str(catalog_identity)
